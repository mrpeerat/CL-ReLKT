...
def negative_mining(all_data_index,triplet_data):
    ...
    for idx,ran in enumerate(language_range):
        if all_data_index < ran:
            q_lan = languages[idx]
            break
    df_para_lan_selected = df_para[f"en-{q_lan}"]

    para_encoded = np.array(df_para_lan_selected[df_para_lan_selected['doc_id']!=doc_id].para_encoded.to_list())
    para_edited = np.array(df_para_lan_selected[df_para_lan_selected['doc_id']!=doc_id].para.to_list())
...
def update_negative_mining():
    all_data_function = [] 
    for idx,item in tqdm(enumerate(all_data[:])):     
        data_temp,error = negative_mining(idx,item)
        if error == 0:
...
training_data = []
training_data_temp = []
f_name = f"models/{corpus}/finetuned_USE_{corpus}_{mode}_en-{arg_config.languages}_{context_mode}_{use_mode}_{arg_config.margin}_{arg_config.batch_update}BatchUpdate_{arg_config.hard}hard_{arg_config.semi_hard}shard_{arg_config.loss_mode}"

if len(f_name) > 254:
    raise Exception(f"File name too long {len(f_name)}/255")
...
for lan in languages: 
    ...
    file_name = f'triplet_en-{lan}_en_top{top_start}-{top_end}_{context_mode}_mlqa.csv'
    df = pd.read_csv(f'data_preprocess/{corpus}/{mode}/triplet/{file_name}')
    # df = df.sample(frac=1)
    df = df.dropna()
    all_data += df.values.tolist()
    len_language+=len(df.values.tolist())
    language_range.append(len_language)

    para_temp = pd.read_csv(f'data_preprocess/{corpus}/train/{corpus.lower()}_para_en-{lan}.csv')
    df_para.update({
        f'en-{lan}':para_temp
    })
...
if __name__ == "__main__":
    ...
    for e in range(num_epoch):
        ...
        if e%arg_config.batch_update == 0: # update all the dataset
            ...
            print('Para encoding.....')
            for lan in languages:
                para_encoded_list = eval_fnc.batch_encode(USE,np.array(df_para[f'en-{lan}']['para'].to_list()),bs=80)
                try:
                    df_para[f'en-{lan}'] = df_para[f'en-{lan}'].drop(columns=['para_encoded'])
                except:
                    pass
                df_para[f'en-{lan}']['para_encoded'] = para_encoded_list.tolist()
        ...
        for i in tqdm(range(num_steps)):
            ...
            if eval_step%100 == 0:
                ...
                for lan in languages: 
                    ...
                    doc_context_encoded = eval_fnc.batch_encode(USE,np.array(df_doc[f'en-{lan}']['doc'].to_list()))
